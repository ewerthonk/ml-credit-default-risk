...
def create_exploratory_dataset(dataset_for_analysis='application_train.csv'):
    ...
    df_exploratory = df_exploratory.replace({'application_{train|test}.csv': 'application_train.csv',
    ...
    df_exploratory['Kurtosis'] = df_exploratory['Column'].map(lambda column: calculate_kurtosis(df, column))
    df_exploratory['CorrWithTarget'] = df_exploratory['Column'].map(lambda column: calculate_correlation(df, column))

    df_exploratory.to_csv(interim_data_directory / 'application_exploratory.csv', index=False)
...
def create_decision_dataset():
    ...
    df_decision['OutliersDecision'] = np.where(df_decision['Column'].isin(cols_with_high_outliers), 'remove', ' ')
